...
if [ $update_version = $online_version ]; then
	...
	#删除旧更新包
	rm -fr /var/lib/jenkins/git_path/.energy_emit_log/update_online.zip
	#不需要更新文件字符串与|串起来
	echo "更新忽略文件列表:"
	...
	set -x
	git diff --name-only $update_version $online_version | grep -vE $deny_files >/var/lib/jenkins/git_path/.energy_emit_log/update.list
	set +x
	echo "最终更新文件列表:"
	cat /var/lib/jenkins/git_path/.energy_emit_log/update.list
	echo "文件打包位置:/var/lib/jenkins/git_path/.energy_emit_log/update_online.zip"
	cat /var/lib/jenkins/git_path/.energy_emit_log/update.list | xargs zip /var/lib/jenkins/git_path/.energy_emit_log/update_online.zip
else
	#删除旧更新包
	rm -fr /var/lib/jenkins/git_path/.energy_emit_log/update_online.zip
	set -x
